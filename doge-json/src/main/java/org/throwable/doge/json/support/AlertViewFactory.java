 ...
 */
public abstract class AlertViewFactory {
	...
	public static void createExceptionAlert(Throwable ex, String header, String content) {
		...
		image.setImage(new Image(AlertViewFactory.class.getResourceAsStream(Constants.DEFAULT_IMG_PATH + "surprise.jpg")));
	...
	public static void createHelpAlert() {
		...
		image.setFitHeight(150);
		image.setFitWidth(150);
		image.setImage(new Image(AlertViewFactory.class.getResourceAsStream(Constants.DEFAULT_LOGO_PATH)));
		grid.add(image, 0, 0);
		...
		Label label = new Label(configuration.getHelpModule().getDescription());
		label.setFont(new Font(18));
		grid.add(label, 0, 5);
		int rowIndex = 6;
